// ─────────────────────────────────────────────────────────────────────────
//  세계관 굽기(bake) — md/ 의 '박제 4종'을 하나의 코드 모듈로 구워낸다.
//
//  왜: Vercel 서버리스 함수는 배포 환경에서 파일(md)을 직접 읽기가 까다롭다.
...
for (const 파일 of 박제문서) {
  try {
    조각.push(fs.readFileSync(path.join(mdDir, 파일), 'utf-8').trim());
  } catch {
...
const out =
  ...
  "// md/ 의 박제 문서가 바뀌면 'npm run bake' 로 다시 만듭니다.\n" +
