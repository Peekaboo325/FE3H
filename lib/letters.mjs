...
[판정 5 — season] [최근 이야기 기록]에 드러난 '지금 계절'을 한 단어로: 봄 / 여름 / 가을 / 겨울. 단서가 없으면 "불명".

[출력 JSON]
{ "owner": { "appeared": true, "confined": false }, "season": "봄", "candidates": [ { "name": "이름", "distance": "거점", "appeared": false, "confined": false } ], "events": [ { "gist": "한 문장", "public": true } ] }`;
}
...
async function 판별하기(owner, candidates, context) {
  ...
  const publicEvents = (Array.isArray(data.events) ? data.events : [])
    ...
    .map((e) => 정리(e.gist))
    .slice(0, 6);
  const season = ['봄', '여름', '가을', '겨울'].includes(정리(data?.season)) ? 정리(data.season) : '';
  return {
    cands,
...
function buildLetterPrompt({
  ...
  현재시기 = '', // 판별이 본문에서 뽑은 현재 계절(봄/여름/가을/겨울) — 사실로 박아 겨울 선입견 차단
}) {
...
- 이미 오간 소식·화제를 되풀이하지 말 것 — 같은 사건을 또 언급하면 실패다. 이번 편지는 새로운 용건을 들고 온다: 발신자 자신의 근황, 다른 사건, 부탁이나 제안, 함께한 기억, 계절 안부 등.`);
  ...
  부분.push(지식);
  if (현재시기)
    부분.push(`[지금 계절 — 확정 사실]\n지금은 ${현재시기}이다. 편지의 날씨·계절감은 반드시 이에 맞춘다 — 다른 계절로 새지 말 것.`);
...
이 편지는 독자가 발신자에게 '이 사람에게 한 통 쓰라'고 직접 청한 것이다. 반드시 한 통을 쓴다 — action을 "silence"로 비우지 말 것. 부칠지(send) 문갑에 남길지(draft)는 위 규칙대로 발신자의 처지·성격이 정한다.`);
  ...
  부분.push(유형안내);
  // 유형 편향 차단 — 강제는 안 하되, 한동안 안 쓴 유형을 상황 닿으면 적극 고르게(초대장 등이 영영 안 나오는 쏠림 방지).
  {
    const 고를수있는유형 = ['letter', 'note', 'official', 'invitation', 'petition', 'warning'];
    const 안쓴유형 = 고를수있는유형.filter((t) => !(최근유형 || []).includes(t));
    부분.push(`[유형 균형 — 한쪽으로 쏠리지 말 것]
- 최근 오간 유형: ${최근유형?.length ? 최근유형.join(', ') : '없음'}.
- 한동안 안 쓴 유형: ${안쓴유형.length ? 안쓴유형.join(', ') : '없음'} — 발신자의 처지·관계·이번 용건에 닿으면 이쪽을 적극 고른다(억지로 끼워 맞추진 말 것).
- 매번 같은 유형으로만 떨어지면 실패다.`);
  }
...
export async function runLetters({ characterId, storyId } = {}) {
  ...
  const { cands: 판별, owner: 주인판정, publicEvents, season } = await 판별하기(owner, candidates, context);
...
export async function runDirectedLetter({ characterId, storyId, receiverId, receiverName } = {}) {
  ...
  const { owner: 주인판정, publicEvents, season } = await 판별하기(owner, [recvCand], context);
