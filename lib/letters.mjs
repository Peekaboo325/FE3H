...
export async function runLetters({ characterId, storyId } = {}) {
  ...
  if (sender.isMob) {
    sender.재료 = `이름 없는 인물 — ${owner.name}의 생활권(${owner.base || '거처'} 부근)에서 일하며 수신자와 엮인 평범한 사람이다. 직함(역할)을 하나 정하고, 그 직분으로서 '자기 용건'을 들고 편지를 쓴다 — 재봉사면 지난번 지어 올린 옷·맡은 비단, 약재상이면 구해 둔 약재·들여온 약초, 요리사면 식단·들인 식재료, 시종이면 맡은 채비처럼, 그 사람의 일이 편지의 중심이다(보고·청·전달·확인 등). 안부만으로 채우지 말 것 — 안부는 곁들임일 뿐이고, 자기 일이 본령이다. 단 궁 안 사정·기밀·정세는 제 직분 밖이라 모르니 아는 척하지 않는다(닿는 범위 안에서만).`;
    sender.name = '';
  }
  if (receiver.isMob) {
