...
- signature: 서명은 직함에 수식을 얹어 자유롭게 — 그 사람의 처지·연륜·마음이 묻어나게 한 줄로 짓는다. 신분에 맞으면 '드림·올림' 같은 맺음 경어도 자연스럽다. 영문 표기 없음.`
    : `[서명]
...
export async function runLetters({ characterId, storyId } = {}) {
  ...
  if (ownerConfined) {
    if (cand구금 || isDead) return { error: '지금은 붓을 들 수 있는 이가 없습니다.' };
    ownerSends = false;
  }
  ...
  // 모브 발신자: 이름 대신 직함(role 칸)을 모델이 짓는다 — 재료는 '주인의 생활권 익명 인물'이라는 처지만.
  if (sender.isMob) {
...
export async function runDirectedLetter({ characterId, storyId, receiverId } = {}) {
  if (!characterId || !receiverId) return { error: '발신자와 수신자를 지목하십시오.' };
  const owner = await getCharacter(characterId);
  if (!owner) return { error: '인물을 찾을 수 없습니다.' };
  if (owner.life_status === 'deceased' || owner.life_status === 'unknown')
    return { error: '세상을 떠난 이는 새 편지를 쓰지 못합니다.' }; // A 사망 = 불가(유서만 별도)
  const receiver = await getCharacter(Number(receiverId));
  if (!receiver) return { error: '수신자를 찾을 수 없습니다.' };
  if (receiver.id === owner.id) return { error: '자신에게는 부칠 수 없습니다.' };
  ...
  // 인연 칸에서 수신자와 맞는 관계 설명(있으면).
  const bond =
    (Array.isArray(owner.bonds) ? owner.bonds : []).find(
      (b) => b?.name && b.name.trim() === (receiver.name || '').trim(),
    ) || null;

  const receiverGone =
    receiver.life_status === 'deceased' || receiver.life_status === 'unknown';

  // 판별 — A(발신자)의 지식 수위·구금 여부 + 공개 사건. (거리는 지정발신엔 무의미.)
  const recvCand = {
    kind: receiverGone ? 'deceased' : 'registered',
    name: receiver.name,
    bond,
    char: receiver,
  };
  ...
  const recv = {
    id: receiver.id,
    name: receiver.name,
    english: receiver.english_name || '',
    재료: 약력문(receiver),
    isMob: false,
  };

  const forceDraftStatus = receiverGone || ownerConfined; // 둘 중 하나면 draft 고정
  ...
  const prompt = buildLetterPrompt({
    ...
    최근유형,
    모브명단,
    forceDraft: receiverGone, // 수신자 사망·실종 → 부치지 못할 편지(텍스트 + 상태)
    confinedSender: ownerConfined && !receiverGone, // A 구금 → 갇힌 자의 편지(텍스트)
