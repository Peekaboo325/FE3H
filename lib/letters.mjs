...
- 발신자는 자기 자신을 경칭으로 부르지 않는다 — 자신이 왕이라도 자기 명령은 '나의 명'이지 '폐하의 명'이 아니다. 자신의 지위·행위·결정은 편지 끝까지 일관되게 1인칭으로 쓴다.
...
function 행만들기(d, { storyId, sender, receiver, replyToId = null, forceWill = false, forceDraft = false, allowDraft = false }) {
  ...
  //  (모브는 한글 직함이 정답이라 제외. 영문명을 모르는 子는 모델의 로마자 표기에 맡긴다.)
  if (!sender.isMob && sender.english && !/[A-Za-z]/.test(signature)) {
    const 한첫 = sender.name.split(/\s+/)[0];
    const 영첫 = sender.english.split(/\s+/)[0];
    if (signature.includes(sender.name)) signature = signature.replace(sender.name, sender.english);
  ...
  const content = 용어교정(본문정리(d?.content));
