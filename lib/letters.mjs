...
function 지식블록(appeared, publicEvents, context) {
  ...
  if (publicEvents.length) {
    return `[발신자가 아는 최근 소식 — 멀리서 소문으로만, 닳고 뭉개진 채 들은 것]\n${publicEvents.map((g) => `- ${g}`).join('\n')}\n(주의: 발신자는 궁 안 사정을 모른다. 위는 막연한 풍문일 뿐 — 구체적 내부 행위·명령·기밀을 아는 척하지 말고 '어수선하다더라'·'무슨 일이 있다던데' 수준의 흐릿한 동요로만 스치듯 다룬다. 편지의 중심은 어디까지나 수신자 안부에 둔다.)`;
  }
...
export async function runLetters({ characterId, storyId } = {}) {
  ...
  if (sender.isMob) {
    sender.재료 = `이름 없는 인물 — ${owner.name}의 생활권(${owner.base || '거처'} 부근)에서 살아가는 평범한 사람이다. 직함(역할)을 하나 정해 그 사람으로서 쓴다. 수신자와는 일상 속에서 스치는 사이다. 자기 처지·생활권에서 알 만한 것(안부·장사·동네 분위기)만 쓰고, 궁 안 사정이나 기밀을 아는 척하거나 정세를 늘어놓지 않는다 — 편지의 본령은 수신자를 향한 안부다(제 신분을 넘는 내용은 어색하다).`;
    sender.name = '';
  }
  if (receiver.isMob) {
