...
import { 서술자클라이언트, 사고옵션 } from './llm.mjs';

// 연출(콘티) 모델 허용목록 — 앱 설정에서 받은 값 검증(임의 값 차단). 기본 Flash.
//  Flash=Gemini(싸고 빠름·연출 약함) / Sonnet·Opus=Claude(연출 강함, 비용↑). CLAUDE.md '콘티 모델 다이얼'.
const 연출모델허용 = new Set(['gemini-2.5-flash', 'claude-sonnet-4-6', 'claude-opus-4-8']);
const 콘티모델 = (m) => (연출모델허용.has(m) ? m : 'gemini-2.5-flash');
const 클로드콘티 = (m) => String(m || '').startsWith('claude');
...
// 연출가 = Claude(Sonnet/Opus) 경로. 작성가드를 system으로, 맥락+1차를 user로. 비스트리밍(콘티는 짧다).
//  ⚠️ 연출가가 Claude면 §3의 'Gemini엔 세계관 금지'는 안 걸리지만, 지금은 세계관·인물을 주지 않고
//     Flash 경로와 같은 '무지한 연출가'로 둔다(2순위=세계관 주입은 빌더와 별도 상의). 여긴 모델만 승격.
async function claudeEnrich(model, { prompt, context, 기록지침 }) {
  const client = 서술자클라이언트(model);
  if (!client || !process.env.ANTHROPIC_API_KEY) return { error: '연출가(Claude) 열쇠가 없습니다.' };
  const 지침블록 = 기록지침 ? `\n[기록자의 지침 — 연출도 이 결을 따른다]\n${기록지침}\n` : '';
  const user =
    `[최근 맥락 — 이어지는 상황·계절의 참고]\n${context || '(아직 쌓인 본문이 없다 — 1차의 방향과 일반 정황으로 연출한다.)'}\n\n` +
    `[작가의 1차 방향]\n${prompt}\n\n[연출 지시문 — 다른 말 없이 지시문만]`;
  try {
    const msg = await client.messages.create({
      model,
      max_tokens: 4000, // 헤드룸(adaptive thinking + ~500자 출력). 콘티는 짧아 실제 소모 작음
      ...사고옵션(model, 'low'), // adaptive thinking + effort 낮음(짧은 설계 과제)
    ...
    console.error('[윤색] Claude 연출 실패:', e?.message || e);
    return { error: '연출을 받지 못했습니다 — 다시 시도하십시오.' };
  }
}

// 윤색 실행 — 1차 프롬프트 + 경량 맥락 → 콘티(2차 산문 지시문). 연출 모델은 앱 설정에서(기본 Flash).
export async function runEnrich({ storyId, prompt, model } = {}) {
  ...
  let text, error;
  if (클로드콘티(연출)) {
    ({ text, error } = await claudeEnrich(연출, { prompt: 씨앗, context, 기록지침 }));
  } else {
