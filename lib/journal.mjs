...
// 최근 회차 묶음에서 '가장 마지막 하루'만 골라 사실 요지로 압축한다(Flash 1콜 — 서신의 판별 분리와 같은 결).
//  핵심 = '날 인식': 화 개수가 아니라 서사의 시간 단절로 하루를 가른다(하루가 여러 화에 걸쳐도 통째로, 그
//  이전 날은 빼서 — 여러 날이 한 일지에 안 섞이게). 산문을 일지에 직접 안 먹여 재연을 막고(§0), POV
//  한정이라 §5 누설 경계도 흡수한다(안 겪은 일은 요지에 안 담겨 못 샌다). 단 절대 날짜는 안 박는다(상대 묶음).
async function 일지요지(context, char) {
  if (!context) return '';
  const prompt = `다음은 "${char.name}"의 이야기에서 최근에 이어진 회차들이다(시간순). 이 가운데 '가장 마지막 하루'에 일어난 일만 골라 사실 요지로 간추려라.

- '가장 마지막 하루' = 마지막으로 날이 바뀐 지점(잠들었다 깨어남·이튿날·새벽·며칠 뒤 같은 '시간의 단절') 이후부터 끝까지를 말한다. 그 이전의 다른 날에 있었던 일은 빼라. (시간 단절이 전혀 없으면 주어진 전부가 하루다.)
- 그 하루가 여러 회차에 걸쳐 있어도 토막 내지 말고 통째로 담되, "${char.name}"이 그 자리에 직접 있어 겪거나 본 일만 추려라.
- 일어난 차례대로(아침→낮→저녁) 사실만 담담히. 장면·대사·묘사를 그대로 옮기지 말 것(다섯 줄 안팎).
- 인물이 말·행동으로 드러낸 사실(갇힌 기간, 스스로 밝힌 이유·동기, 관계 등)은 추측이 아니라 사실이니 정확히 살려라. 기록에 적힌 기간·내력('한 달 남짓'·'다섯 해' 등)도 그대로 옮기되, 서로 다른 일(예: 오랜 실종과 최근의 감금)을 뭉뚱그려 섞지 마라.
- 그가 현장에 없던 일, 그로선 모르는 일은 빼라.
- 그가 직접 겪은 일이 없으면 아무것도 적지 말고 빈칸으로 둔다.
- 버릴 것은 절대 날짜(제국력 ○일)·미사여구·기록에 없는 네 해석뿐이다. 기록에 없는 내력·동기·숫자를 지어내 보태지 마라. (하루 안의 시간 차례는 위처럼 살린다.)
...
[오늘 있었던 일 — 사실 요지${있음 ? '' : ' (이렇다 할 일은 없었다)'}]
${있음 ? 요지 : '(특별한 일은 없었다. 그저 여느 날의 하루를 적는다.)'}
- 위는 '오늘 하루'에 있었던 일의 사실 요지일 뿐, 장면이 아니다. 그대로 옮기거나 장면·대사로 펼쳐 재연하지 마라 — 그건 일기가 아니라 본문 복기다.
- 하루 안에 일이 여럿이어도 시간 차례(아침→저녁)는 지키되, 그 일들을 차례차례 다 옮겨 적지는 마라. 그중 마음에 가장 남은 것을 중심으로, 나머지는 스치듯 둔다.
- 일기는 사건의 재생이 아니라, 그 일이 네 안에 남긴 것을 되새기는 글이다.
...
- 소소한 일상이 바탕이다. 아침에 무엇을 먹었는지, 어떤 소임을 보았는지, 누구와 스쳤는지 — 너의 신분·거점·지금 처지가 그 하루를 정한다(수도원의 기사라면 훈련과 순찰, 영애라면 서신과 자수, 갇힌 몸이라면 차가운 감방).
- 비율은 그날에 맡긴다: 마음을 뒤흔든 큰일이 있었다면 그것이 글의 중심이 되어도 좋다 — 단 재연이 아니라 되새김으로, 소소한 일상 한두 조각으로 숨 쉴 틈을 둔다. 큰일이 없던 한가한 날이면 그 일상이 곧 일기의 전부다.
