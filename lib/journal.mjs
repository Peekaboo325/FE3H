...
//   · ⚠️ 본문 산문을 일지에 직접 안 먹인다 — 먼저 Flash로 '겪은 일의 사실 요지'로 압축하고(2콜),
//     일지는 그 요지로만 쓴다(서신의 판별/생성 분리와 같은 결). 산문을 못 보면 베낄(재연) 수 없다.
...
const JOURNAL_RAW = 1; // 한 일지 = 가장 최근 한 화(=하루). 여러 날을 한 일지에 담으면 시간이 섞여 깨진다.
...
// 최근 창 원문을 '글쓴이가 직접 겪은 일의 사실 요지'로 압축한다(Flash 1콜 — 서신의 판별 분리와 같은 결).
//  일지에 장면 산문을 직접 안 먹이려는 것. 산문을 못 보면 그걸 베껴 재연할 수가 없다(§0).
//  POV 한정이라 §5 누설 경계도 흡수 — 안 겪은 일은 요지에 안 담겨 일지에 못 샌다(등장 판별을 따로 안 짜도 됨).
async function 일지요지(context, char) {
  if (!context) return '';
  const prompt = `다음은 요즘 포드라에 있었던 일들의 기록이다. 인물 "${char.name}"의 시선에서, 그가 그 자리에 직접 있어 겪거나 본 일만 골라 사실로만 간추려라.
- '무슨 일이 있었나'만 담담히. 장면·대사·묘사를 옮기지 말 것(서너 줄 안팎).
- 일이 일어난 차례와 시간의 흐름(아침·저녁·이튿날 등)은 살려서 순서대로 적어라 — 뒤섞지 말 것.
- 그가 현장에 없던 일은 빼라 — 그로선 모르는 일이다.
- 그가 직접 겪은 일이 없으면 아무것도 적지 말고 빈칸으로 둔다.
- 절대 날짜(○일)·심리 추측·미사여구는 버린다(시간 차례는 위처럼 살린다).
...
[가장 최근에 네게 있었던 일 — 사실 요지${있음 ? '' : ' (이렇다 할 일은 없었다)'}]
${있음 ? 요지 : '(특별한 일은 없었다. 그저 여느 날의 하루를 적는다.)'}
- 위는 '무슨 일이 있었나'의 사실 요지일 뿐, 장면이 아니다. 이걸 그대로 옮기거나 장면·대사로 펼쳐 재연하지 마라 — 그건 일기가 아니라 본문 복기다.
- 이건 '오늘'의 일이다. 여러 날을 하루로 뭉뚱그리거나 시간 차례를 뒤섞지 마라(요지에 아침→저녁 흐름이 있으면 그 차례를 지킨다). 지난 일은 지난 일로(‘며칠 전’·‘어제’) 두고, 오늘 일을 오늘로 적는다.
- 일기는 사건의 재생이 아니라, 그 일이 네 안에 남긴 것을 되새기는 글이다. 요지에 여러 일이 있어도 차례로 늘어놓지 말고, 그중 마음에 가장 남은 하나에 머물러라.
