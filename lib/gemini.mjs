...
//
//  callGemini     = 일반 호출 (재시도·차단기 포함, 결과 한 번에)
//  callGeminiStream = SSE 스트리밍 호출 (onChunk 콜백으로 델타 실시간 전달 — 교정 패스용)
//
...
// Gemini SSE 스트리밍 — onChunk(text) 콜백으로 델타를 실시간 전달.
//  streamGenerateContent?alt=sse: 각 이벤트가 "data: {...}" 형태로 오고,
//  candidates[0].content.parts[].text 에 텍스트 델타가 담긴다.
//  ⚠️ thinkingBudget > 0이면 thinking 파트도 흘러오므로 text 없는 파트는 건너뜀.
//  재시도·차단기는 callGemini에만(스트리밍은 연결 직후 실패가 전부라 retry 불필요).
export async function callGeminiStream(prompt, opts = {}, onChunk) {
  const {
    temperature = 0.3,
    maxOutputTokens = 2048,
    thinkingBudget = 0,
  ...
  if (Date.now() < 차단해제시각) throw new Error('차단기 작동 중 — 잠시 후 재시도');

  const generationConfig = { temperature, maxOutputTokens, thinkingConfig: { thinkingBudget } };
  ...
  if (!r.ok) {
    const body = await r.text().catch(() => '');
    throw new Error(`[${tag}] Gemini 스트리밍 ${r.status}: ${body}`);
  }
