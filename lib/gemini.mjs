...
import { 기록사용량 } from './db.mjs'; // API 사용량 집계(빌더 진단) — 모든 Gemini 호출이 이 통로를 거치므로 여기서 한 번에 잡는다
...
export async function callGemini(prompt, opts = {}) {
  ...
  for (let 시도 = 0; 시도 < 최대시도; 시도++) {
    ...
    try {
      ...
      // 사용량 집계 — Gemini는 캐시 개념이 없어 in/out만(생각 토큰은 thinkingBudget=0이라 ~0).
      const um = d?.usageMetadata;
      if (um) await 기록사용량(model, { in: um.promptTokenCount || 0, cw: 0, cr: 0, out: um.candidatesTokenCount || 0 });
      return {
        text: text.trim() || null,
        finish,
