...
[항목별 한 줄 평]
- 각 능력에 짧은 명사구 하나(쉼표로 두 구를 잇지 말 것). 좁은 칸에 인라인으로 들어가니 공백 포함 6~12자로 짧게, 그리고 세 항목의 길이가 들쭉날쭉하지 않게 비슷한 밀도로 맞춘다.
- 한 단어짜리 호칭·별명만 달랑 두지 말고 짧은 수식을 붙여 명사구로 만든다. 12자를 넘기지 말 것(넘으면 화면에서 잘린다).
...
[행동 양상 / 잠재 심리]
- 이 둘은 인물의 '지금'을 읽는다(중요): 약력은 지속되는 바탕일 뿐, 최근 맥락(본문)에서 이 인물이 무슨 일을 겪고 어떻게 반응하고 있는지를 '우선' 근거로 삼는다. 같은 사람이라도 국면이 그를 다르게 드러내니, 진도가 나가면 이 분석도 달라져야 한다 — 본문이 쌓였는데 약력만 반복하면 실패다.
...
[기반 종합]
- foundation: 입지(정치적 영향력·신분)·재력(물질적 형편)·명망(이름의 무게) 세 축을 아울러, 이 인물이 세상에서 어떤 '위치'에 놓였는지 3~4문장으로 짚는다.
- 세 축의 높낮이 '관계'를 읽고(엇갈리면 그 긴장을, 나란하면 그 처지를), 그 위치가 어떻게 작동하는지까지 쓴다 — 누구를 거느리고 무엇에 매이며 어떤 정치적·물질적 제약 아래 있는지.
- ⚠️ 감정·고뇌·집착 같은 '내면'은 기반에 쓰지 않는다 — 그건 행동 양상·잠재 심리 소관이다.
- ⚠️ 본문·약력에 실제 근거가 없는 평가를 지어내지 않는다. '권력자는 으레 직무에 소홀하다' 같은 트로프·일반론으로 처지를 멋대로 깎거나 추측하지 말 것 — 글에서 확인되는 사실만 쓰고, 근거가 없으면 위치의 골자만 담담히 적는다.
- 수치를 그대로 옮기지 말고, 평가·예언·미사여구 없이 담담히.
