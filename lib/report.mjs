...
const RECENT_RAW = 14; // 보고서 맥락에 원문으로 넣을 최근 턴 수(Gemini 여유 있어 넉넉히)
...
//   ① 큰 줄기(거시 요약) + ② 큰 줄기에 아직 안 녹은 회차별 요약 + ③ 최근 원문.
// ⚠️ 읽기 전용 — 없는 회차 요약을 새로 '생성'하진 않는다(§9 모르는 새 API 폭주 방지).
//    그래서 요약이 비어 있어도 ③ 최근 원문으로 최신 맥락은 늘 확보된다.
async function 맥락모으기(storyId) {
  ...
  const cut = Math.max(0, turns.length - RECENT_RAW); // 최근 원문 창의 시작
  const arcThrough = summaryTurnId || 0; // 큰 줄기가 흡수한 지점

  // ② 최근 원문 밖 + 큰 줄기에도 안 녹은(=중복 아닌) 회차 요약만 시간순으로.
  const 회차요약 = [];
  if (hasSummaryCol) {
    for (let i = 0; i < cut; i++) {
      const t = turns[i];
      if (t.role === 'assistant' && t.summary?.trim() && (t.id ?? 0) > arcThrough) {
        회차요약.push(t.summary.trim());
      }
    }
  }
