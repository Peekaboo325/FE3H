...
// 일시적 과부하(503·429 등) 재시도 — 보고서는 무거운 요청(사고+큰 맥락+긴 출력)이라 과부하 모델에 잘 튕긴다.
const 보고서_일시적 = new Set([429, 500, 503, 504]); // 재시도 대상(영구 에러·파싱 실패는 즉시 반환)
const 보고서_잠깐 = (ms) => new Promise((res) => setTimeout(res, ms));
...
async function callGeminiJSON(
  ...
  const 백오프 = [1500, 3000]; // 1.5초 → 3초 (요약보다 길게 — 무거운 호출이라)
  for (let 시도 = 0; 시도 <= 2; 시도++) {
    try {
      ...
      if (!r.ok) {
        ...
        // 일시적(503 등)이고 시도 여유 있으면 잠깐 쉬고 재시도, 아니면 폴백.
        if (보고서_일시적.has(r.status) && 시도 < 2) {
          console.error(`[보고서] Gemini ${r.status} 일시적 — 재시도 ${시도 + 1}/2`);
          await 보고서_잠깐(백오프[시도] ?? 3000);
          continue;
        }
      ...
      if (시도 < 2) {
        console.error(`[보고서] Gemini 오류(재시도 ${시도 + 1}/2):`, e?.message || e);
        await 보고서_잠깐(백오프[시도] ?? 3000);
        continue;
      }
  ...
  return { error: 'Gemini 재시도 소진(과부하 지속)' };
}
...
[항목별 한 줄 평]
- 각 능력에 단일 명사구 하나(쉼표(,)로 두 구를 잇지 말 것). 길이는 공백 포함 25자 미만이되, 너무 짧게 끝내지 말고 수식을 얹어 18~24자로 밀도 있게 쓴다.
...
[행동 양상 / 잠재 심리]
- personality(행동 양상): 약력의 〈성향〉을 그대로 옮겨 적지 말 것. 흩어진 성향을 하나의 패턴으로 종합하고, '어떻게·왜 그렇게 행동하는가'의 메커니즘을 짚는다. 본문 맥락이 있으면 행적에서 드러난 모습을 근거로. 3~5문장.
