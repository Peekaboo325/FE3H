...
//  ⚠️ gemini-2.5-flash는 기본 'thinking'이 출력 토큰을 먹어 본문이 잘릴 수 있다 →
//     이 작업(분류·구조화)은 thinking이 불필요하므로 끈다(thinkingBudget:0) + 한도 넉넉히.
async function callGeminiJSON(prompt, { temperature = 0.85, maxOutputTokens = 8192 } = {}) {
  ...
  try {
      ...
      `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${key}`,
      {
        ...
        body: JSON.stringify({
          ...
          generationConfig: {
            ...
            thinkingConfig: { thinkingBudget: 0 }, // 생각 끔 — 출력 토큰 절약(잘림 방지)
          },
        }),
      },
    );
    ...
    if (!text.trim()) {
      ...
      if (finish === 'MAX_TOKENS') return { error: '응답이 토큰 한도에서 잘렸습니다 — 다시 시도해 주세요.' };
