...
import { injectionIndices, ensureSummariesAt, assembleFromIndices } from './chronicle.mjs';

// 원문으로 그대로 보낼 최근 턴 수 = 최근 2화(프롬프트+본문 4턴) + 이번 새 입력.
export const KEEP_TURNS = 5;
// 원문 창 밖에서, 요약으로 주입할 최근 화 수(상한). 그 이전은 앵커로만 불러온다.
export const KEEP_SUMMARY = 20;
...
export async function prepareConversation(storyId, clientMessages) {
  ...
  // 옛 화 전부가 아니라 '최근 KEEP_SUMMARY화'만 요약으로 주입(그 이전은 앵커로).
  const idxs = injectionIndices(turns, cutoff, KEEP_SUMMARY);
  await ensureSummariesAt(turns, idxs);
  const summary = assembleFromIndices(turns, idxs);
