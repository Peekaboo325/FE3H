...
import { ensureSummariesAt, assembleFromIndices, loadSummaryContext } from './chronicle.mjs';
...
export async function prepareConversation(storyId, clientMessages) {
  ...
  //  요약 시 인물·큰줄기 맥락을 줘 막연한 호칭("죄인" 등)을 이름으로 통일.
  let unarced = zone.filter((i) => (turns[i].id ?? 0) > arcThrough);
  await ensureSummariesAt(turns, unarced, await loadSummaryContext(storyId));
