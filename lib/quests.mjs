// ─────────────────────────────────────────────────────────────────────────
//  임무 장부 발급소 — Gemini Flash가 인물이 '스스로 꾸미는 계획' 4개를 짓는다(유형 조합은 매번 다르게).
//
...
//  typePlan = 이번 발급의 유형 4개(코드가 매번 무작위 배정 → 결정론적 모드붕괴 차단). 모델은 그 유형에 맞춰 내용만 짓는다.
export function buildQuestsPrompt(char, { context, typePlan = [] } = {}) {
  return `너는 포드라의 '왕실 분석관'이다. 인물 "${char.name}"의 임무 장부를 발급하라.
임무 = 이 인물이 지금 시점에 스스로 세우는 계획. 정확히 4개를 만든다.
출력은 모두 자연스러운 한국어(올바른 띄어쓰기·문법)로 쓴다.

[이번 발급의 유형 배정 — 반드시 이 순서대로]
- 4개 임무의 유형을 순서대로 다음으로 한다: ${typePlan.join(' → ')}.
- 각 임무를 배정된 유형의 결에 맞게 쓰되, 내용은 인물의 '지금 국면'(최근 맥락)에 어울리게 한다. 배정된 유형 중 현재 흐름과 어긋나는 것이 있으면 그 마찰·부조화를 자연스러운 갈등으로 살린다.
...
- 내용 적응: 유형은 위 '유형 배정'을 따르되, 각 임무의 내용은 최근 맥락의 결(전쟁·위기·평화·축제·일상 등)을 반영한다 — 같은 유형이라도 지금 상황에 맞게.
- 지목: 설명 안에 구체적인 이름(인물·장소·물건)을 적어 의도를 땅에 붙인다. 최근 맥락에 등장한 이름을 우선한다.
- type: 위 '유형 배정'에 지정된 값을 그 순서대로 그대로 쓴다(임의로 바꾸거나 재배열하지 말 것).
- name(제목): 한국어 명사형 종결, 15자 이내.
- description(설명): 1인칭 속마음 독백 한 문장, 70자 이내. 남에게 보고하거나 규칙을 적는 게 아니라 인물이 제 속으로 다지는 혼잣말이다 — 사적인 결심·다짐·망설임·자문이 묻어나는 어조로, 그 인물의 말버릇으로 맺는다. 딱딱한 당위형('~해야 한다' 류)으로 굳지 말 것. 임무마다 맺는 어조를 다르게(전부 같은 어미로 끝내면 실패). 배경 설명을 늘어놓지 말고 '내 계획' 자체에 집중한다.
...
{ "quests": [ { "type": "${typePlan[0] || '의무'}", "name": "제목", "description": "1인칭 독백", "reward": "보상" } ] }`;
}

// LLM 출력의 들쭉날쭉함을 다듬어 화면이 믿고 쓸 수 있는 모양으로 고정한다.
function normalizeQuests(d, typePlan = []) {
  const 정리 = (s) => String(s ?? '').replace(/[[\]【】]/g, '').replace(/\s{2,}/g, ' ').trim();
  const arr = Array.isArray(d?.quests) ? d.quests : [];
  return arr
    .map((q, i) => ({
      // 유형은 코드가 배정한 계획을 그대로 박는다(모델이 어기든 무시 — 결정론적 동일 라인업 방지의 핵심).
      type: typePlan[i] || (QUEST_TYPES.includes(정리(q?.type)) ? 정리(q.type) : '돌발'),
      name: 정리(q?.name),
...
// 이번 발급의 유형 계획 — 5종을 섞어 4개(서로 다른 유형, 무작위 순서). 호출마다 달라져 동일 라인업이 안 나온다.
function pickTypePlan() {
  return [...QUEST_TYPES].sort(() => Math.random() - 0.5).slice(0, 4);
}
...
export async function runQuests({ characterId, storyId } = {}) {
  ...
  const typePlan = pickTypePlan(); // 호출마다 무작위 — 동일 라인업·순서 차단
  const prompt = buildQuestsPrompt(char, { context, typePlan });
  ...
  const quests = normalizeQuests(data, typePlan);
