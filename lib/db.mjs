...
// 한 이야기의 '마지막 N턴'만 — 스트리밍 끊김 복구 확인용(전 회차 X). 표시본 칸까지 담아 그대로 패치 가능.
//   n=1: 조수 본문 한 칸. n=2: 유저(초안/연출)+조수 두 칸 — 실행 후 초안 칸 id까지 함께 복구하려 꼬리를 조금 더 받는다.
export async function loadLastTurn(storyId, n = 1) {
  const c = getClient();
  if (!c || !storyId) return { turns: [], error: null };
  const lim = Math.max(1, Math.min(Number(n) || 1, 4)); // 폭주 방지 상한(꼬리만)
  ...
  return { turns: (data ?? []).reverse(), error: null }; // id 내림차순으로 받아 시간순으로 되돌린다
}
...
// 턴 하나를 직접 심는다(모델 호출 없이) — 친필. 저장된 행(id 포함)을 돌려줘 클라가 그 자리에 버튼을 붙이게.
export async function insertTurn(storyId, role, content) {
  const c = getClient();
  if (!c) return { error: 'Supabase 미설정' };
  if (!storyId || !String(content ?? '').trim()) return { error: '심을 내용이 없습니다.' };
  const { data, error } = await c
    .from('turns')
    .insert({ role, content, story_id: storyId })
    .select('id, role, content')
    .single();
...
// 여러 화를 한 번에 심는다(모델 호출 없이) — 파일 반입(대량). 배열 순서대로 삽입해 화 차례를 지킨다.
export async function insertTurns(storyId, rows) {
  const c = getClient();
  if (!c) return { error: 'Supabase 미설정' };
  if (!storyId || !Array.isArray(rows) || !rows.length) return { error: '심을 내용이 없습니다.' };
  const clean = rows
    .filter((r) => r && typeof r.content === 'string' && r.content.trim())
    .map((r) => ({ role: r.role === 'user' ? 'user' : 'assistant', content: String(r.content), story_id: storyId }));
  if (!clean.length) return { error: '심을 내용이 없습니다.' };
  const { error } = await c.from('turns').insert(clean); // 배열 순서대로 = id 오름차순 = 화 차례 보존
  if (error) return { error: error.message };
...
// ── API 사용량 집계(빌더 진단) ─────────────────────────────────────────────
//  settings 표 재사용 — key='usage' 한 줄에 모델별 누적 {calls,in,cw,cr,out(+ DeepSeek는 *_peak)} JSON(스키마 변경 없음).
...
// DeepSeek 피크·밸리 요금 — 피크 시간대엔 '모든 항목 2배'(2026-07-15~ 발표: 7월 중순).
//  피크(UTC): 1:00–4:00·6:00–10:00 = 한국(UTC+9) 10–13시·15–19시. 시행 전엔 평시가.
//  ⚠️ 개별 호출 시각을 저장 안 하므로, 기록하는 '지금'이 피크면 그 회차 토큰을 *_peak에도 함께 쌓아 둔다(화면이 그만큼 가산).
const 피크시행 = Date.UTC(2026, 6, 15); // 2026-07-15 00:00 UTC. 시행일/해가 바뀌면 이 값만 수정.
function 딥시크피크지금() {
  const now = new Date();
  if (now.getTime() < 피크시행) return false; // 시행 전 — 평시가
...
export async function 기록사용량(model, u) {
  ...
  try {
    ...
    cur.cr += u.cr || 0;
    cur.out += u.out || 0;
    // 피크 시간대의 DeepSeek 토큰은 *_peak에도 누적(화면에서 2배로 반영). 그 외 모델·평시엔 미적용.
    if (String(model).startsWith('deepseek') && 딥시크피크지금()) {
      cur.in_peak = (cur.in_peak || 0) + (u.in || 0);
      cur.cw_peak = (cur.cw_peak || 0) + (u.cw || 0);
      cur.cr_peak = (cur.cr_peak || 0) + (u.cr || 0);
      cur.out_peak = (cur.out_peak || 0) + (u.out || 0);
    }
    all[model] = cur;
    await c
      .from('settings')
