...
// 한 턴의 본문만 가져온다(연대 문헌 '다시 기록'=단일 화 재요약용).
export async function getTurnContent(id) {
  const c = getClient();
  if (!c || !id) return null;
  const { data, error } = await c.from('turns').select('content').eq('id', id).maybeSingle();
  if (error) {
...
export async function setGuidance(text) {
  const c = getClient();
  if (!c) return { ok: false, error: '보관소가 설정되지 않았습니다.' };
  const { error } = await c
    .from('settings')
    .upsert(
      { key: 'guidance', value: text ?? '', updated_at: new Date().toISOString() },
      { onConflict: 'key' },
    );
  if (error) {
