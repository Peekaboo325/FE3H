...
import { loadCharactersForInjection, getGuidance } from './db.mjs';
...
export async function runIdeate({ storyId } = {}) {
  ...
  const [{ messages, summary }, 인물들, 지침] = await Promise.all([
    prepareConversation(storyId, []), // ①큰 줄기+②최근 요약(summary) + ③최근 원문(messages)
