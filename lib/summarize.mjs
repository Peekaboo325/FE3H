      ...
      const r = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${key}`,
        {
          ...
          body: JSON.stringify({
            ...
            // ⚠️ gemini-2.5-flash는 답 전에 '사고(thinking)' 토큰을 쓰는데 그게 maxOutputTokens를
            //    같이 깎는다 → 예산이 작으면 요약이 중간에 잘린다. 요약·앵커는 기계적 처리라
            //    사고가 불필요(§3) → 꺼서(0) 예산을 통째로 답에 쓴다(잘림 방지·더 빠르고 쌈).
...
${새기록}`;
  ...
  return callGemini(prompt, { temperature: 0.3, maxOutputTokens: 1200 });
}
