...
// 한 화(장면)를 '연대 문헌'에 남길 기록으로 정제한다 — 화별 1:1 요약(표준 규칙).
//  · 서술자 본문(STORY)만이 실재. 작가 지시는 의도일 뿐, 본문에 실현된 것만.
//  · 사건·인과 + 명시적 관계·상태 변화만. 심리 추측·미사여구·날짜·메타지시는 버린다.
export async function summarizeEpisode(directive, body) {
  if (!body?.trim()) return null;

  const prompt = `너는 포드라의 '왕실 사관'이다. 아래 한 화(장면)를 '연대 문헌'에 남길 기록으로 정제하라.

[무엇이 실재인가]
- 서술자 본문(STORY)만이 포드라에 실재한다. 작가 지시(DIRECTIVE)는 의도일 뿐이며, 본문에 실제로 실현된 것만 기록한다.
- "더 길게/다시/수정" 같은 메타 지시나 잡담은 기록하지 않는다.

[요약 규칙]
- 한국어. 건조하고 사실 중심으로, 'A가 B하다' 꼴.
- 사건의 인과와 인물의 행동·결정·결과를 2~4문장으로.
- 심리 추측("슬퍼 보인다") 금지 — 확정된 행동·발화·상태만.
- 수식어·미사여구·날짜는 덜어낸다. 근거 없는 것은 적지 않는다.
