...
// 반환: { block, episodes, missNums, briefFailed }
//   · episodes  = 실제로 찾아 주입한 회차 번호(확인 자취용).
//   · missNums  = 지목했으나 아직 존재하지 않는 회차(없는 화를 가리킴 → '되짚지 못함' 안내용).
//   · briefFailed = 회차는 있었으나 Flash 추려내기가 실패함(주입 못함 → 안내용).
...
export async function buildAnchorContext(storyId, userInput, episodeNums) {
  const 없음 = { block: null, episodes: [], missNums: episodeNums || [], briefFailed: false };
  ...
  const foundNums = episodes.map((e) => e.n);
  const missNums = episodeNums.filter((n) => !foundNums.includes(n)); // 지목했으나 없는 회차
  if (!episodes.length) return { block: null, episodes: [], missNums, briefFailed: false };

  const brief = await pickAnchorBrief(userInput, episodes);
  if (!brief) return { block: null, episodes: [], missNums, briefFailed: true }; // 추려내기 실패
...
//   · '잠듦(is_active === false)'인 인물만 소환 주입한다.
//     활성(true)·미설정(null/undefined)은 이미 상주로 주입되므로(charContext와 동일 기준) 'here'로만 알린다.
//   · 캐시 경계 '뒤'에 주입(매 입력마다 달라짐) → 상주 인물·세계관 캐시를 안 깬다.
//  반환: { block, chars, here, miss }
//   · chars = 실제로 소환한 퍼스트네임(확인 자취용).
//   · here  = 지목됐으나 이미 함께 있는(상주) 인물 — 주입은 생략, 안내만.
//   · miss  = "<이름> 등장"이라 적었으나 명부에서 못 찾은 이름(되짚지 못함 안내용).
//  → 조용한 실패를 없애, 작가가 '소환됨 / 이미 함께 / 명부에 없음'을 토스트로 구분해 본다.
const 퍼스트 = (name) => (name ?? '').trim().split(/\s+/)[0] ?? '';
const 정규이스케이프 = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
const 조사털기 = (s) =>
  (s || '').replace(/(이|가|은|는|을|를|와|과|도|만|에게|한테|께서|이라|라)$/u, '');

export async function buildCharacterAnchorContext(storyId, text) {
  const 없음 = { block: null, chars: [], here: [], miss: [] };
  const t = String(text || '');
  ...
  const picked = []; // 소환할 잠든 인물
  const here = []; // 이미 함께 있는(상주) 인물 — 안내만
  for (const c of characters) {
    ...
    if (c.is_active === false) {
      if (!picked.includes(c)) picked.push(c); // 잠듦 → 소환
    } else if (!here.includes(fn)) {
      here.push(fn); // 활성·미설정 → 이미 상주
    }
  }

  // "<이름> 등장"이라 적었으나 어떤 등록 인물도 안 걸린 이름 = 명부에 없는 인물.
  const miss = [];
  const seen = new Set();
  const runRe = /([가-힣]{2,12})\s*등장/g;
  let m;
  while ((m = runRe.exec(t))) {
    const run = m[1];
    const hit = characters.some((c) => {
      const fn = 퍼스트(c?.name || '');
      return fn && run.startsWith(fn);
    });
    if (hit) continue;
    const disp = 조사털기(run);
    // '들'로 끝나면 무리(병사들 등) — 인명이 아니므로 오탐 제외.
    if (disp.length >= 2 && !disp.endsWith('들') && !seen.has(disp)) {
      seen.add(disp);
      miss.push(disp);
    }
  }
  ...
  return {
    block: picked.length ? buildSummonedCharacters(picked) : null,
    chars: picked.map((c) => 퍼스트(c.name)),
    here,
    miss: miss.slice(0, 5),
  };
}
