...
// ── 문헌(대륙 문헌) 앵커 ──────────────────────────────────────────────────
//  "제N권 참조" (권 번호) 또는 "(제목 참조)" (괄호 안 제목)로 문헌을 지목한다.
//  (습관 대비: '제N장'도 받아준다.) 문헌은 짧고 정제돼 있어 Flash로 추리지 않고
//  그 항목을 그대로 주입한다. (특히 '잠듦' 문헌을 그때만 되살리는 데 쓸모.)
...
export function parseLoreAnchors(text) {
  ...
  const numRe = /제\s*(\d+)\s*(?:권|장)\s*(?:참고|참조)/g; // 권 기준(장도 받아줌)
  ...
  while ((m = titleRe.exec(text))) {
    const t = m[1].trim();
    if (/^제\s*\d+\s*(?:권|장)$/.test(t)) continue; // 권 번호 참조는 위에서 처리됨
...
export async function buildLoreAnchorContext(storyId, { numbers, titles }) {
  ...
  if (!storyId || (!numbers.length && !titles.length)) return 없음;
  const { lore } = await listLore(storyId); // sort_order, id 순 = 제N권
  ...
  const entries = [...picked.values()].map((e) => ({
    e,
    권: lore.findIndex((x) => x.id === e.id) + 1,
  }));

  const blocks = entries.map(({ e, 권 }) => {
    const cat = e.category ? ` (${e.category})` : '';
    return `## 제${권}권 · ${e.title}${cat}\n${e.body || ''}`;
  });
  ...
  return {
    block:
      ...
      blocks.join('\n\n'),
    items: entries.map(({ e, 권 }) => ({ n: 권, t: e.title })),
  };
}
