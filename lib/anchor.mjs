...
//   ① 영역 + 권번호 — "세력 제1권 참고" · "가문 1권 참조" · "세력가문 제1권" · "세력·가문 1권"
//      (영역 토큰은 반쪽·붙임·중점 모두 허용, '제' 생략 가능, '권'/'장' 받음.)
//   ② 제목 — "(퍼거스 신성 왕국 참고)" (괄호). 같은 이름이 여럿이면 전부 되짚는다.
//  문헌은 짧고 정제돼 있어 Flash로 추리지 않고 그대로 주입한다('잠듦' 문헌 임시 소환에 쓸모).

// 다섯 영역(대륙 문헌 분류) — UI(Lore.tsx TOPICS)와 일치 유지.
const 영역들 = ['세력 · 가문', '지역 · 지리', '역사 · 사건', '제도 · 문화', '문장 · 유산'];
...
// 영역 토큰(세력/가문/세력가문/세력·가문 …) → 영역 제목. 못 찾으면 null.
function 영역찾기(token) {
  const t = 무공백(token);
