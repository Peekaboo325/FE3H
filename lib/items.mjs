...
import { ITEM_ICONS, ICON_BY_LABEL, ICON_KEYS } from './itemIcons.mjs';

const 탐색수 = 3; // 한 번 탐색에 찾아내는 물건 수
const 그림라벨 = ITEM_ICONS.map((i) => i.label).join(', '); // AI가 고를 그림 후보(보유 명단)
...
- icon: 아래 [그림 후보]는 '이미 지은 물건에 그림을 매칭'할 때만 쓴다. 목록에 어떤 단어가 있다는 이유로 그 물건을 짓지 말 것 — 물건은 오직 약력·맥락에서 길어 올린다. (예: 목록에 음식이 많다고 음식을 자주 만들지 말 것.) 물건을 다 지은 뒤, 그 물건과 '가장 비슷한' 것 하나를 골라 그대로 적는다. 꼭 들어맞지 않아도 가장 가까운 것으로(딱 맞는 게 없으면 가장 근접한 갈래로). 목록에 없는 단어는 쓰지 말 것.
...
//  icon: AI가 고른 그림 라벨(또는 키)을 보유 명단의 key로 환원. 못 맞추면 비움(화면은 폴백).
function normalizeItems(d) {
  ...
  const 아이콘키 = (raw) => {
    ...
    return undefined; // 명단 밖 → 폴백(빈 칸)
  };
  const arr = Array.isArray(d?.belongings) ? d.belongings : [];
  const base = Date.now().toString(36);
  return arr
    .map((b, i) => ({ id: `i${base}${i}`, name: 정리(b?.name), comment: 정리(b?.comment), icon: 아이콘키(b?.icon) }))
    .filter((b) => b.name)
    .slice(0, 탐색수);
}
