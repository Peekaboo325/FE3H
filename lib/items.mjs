...
소지품은 이 인물이 '지금 통과하고 있는 국면'의 기록이다 — 본디 어떤 사람인가(약력)는 바탕일 뿐, 지금 무엇을 지니고 있는가는 아래 [최근 맥락]의 처지·국면이 정한다.

[최근 맥락 — 지금 이 인물의 처지·국면 (무엇을 지니는지 좌우)]
${context || '(아직 쌓인 본문이 없다 — 약력에 기대 찾아낸다.)'}

[약력 — 바탕(본디 어떤 사람인가)]
${약력문(char)}
(표기·정체는 게임 원작보다 위 커스텀 약력을 우선한다. 단 '지금 무엇을 지니는가'는 위 맥락이 정한다.)
...
- comment(설명): 한 문장, 짧을수록 좋다(최대 25자이되 꽉 채우려 하지 말 것). 물건의 상태·흔적·질감 가운데 **단 하나의 관찰**만 적고 거기서 끝낸다. 특징을 여러 개 나열하지 말고, 원인을 깔고 결과를 잇는 식으로 문장을 부풀리지 않으며, 이름에 이미 담긴 특징은 설명에서 되풀이하지 않는다. 화법은 추론(~듯하다)·감각(~느껴진다)·상태(~있다) 중 하나. 점 두 개(..) 금지.
- 보여주되 말하지 않는다(가장 중요): 설명의 주어는 늘 '물건'이다 — 주인이 무엇을 했는지(행위)·느꼈는지(심정)·어떤 사람인지가 아니라, 물건이 어떤 상태인지로만 문장을 맺는다. 주인의 성격·과거·동기, 또 이 물건이 주인에게 오게 된 개인적 사연(누가 줬나·무슨 일로 생겼나)은 지어내지 않는다. 인물의 결은 물건의 상태로 은근히 배어 나오게 두고 읽는 이가 미루어 느끼게 한다 — 결론을 대신 적지 않는다. '~듯하다'를 붙여도 주인의 행위·심중·사연을 끌어들이면 위반이다.
  - 단, 물건이 '어느 땅·문화·재질의 것인가'(지역 산물·세공·풍토)는 개인 사연이 아니라 물건의 정체다 — 이건 막지 않으며, 이름에 담아 풍미를 낸다.
...
- ${탐색수}점 중 적어도 1점엔 아래 [고유명사 표기]의 지역·문화·세력 결을 '이름'에 녹인다(어느 땅의 산물·세공인가) — 지명을 기계적으로 붙이지 말고 풍미로. 이건 위의 '내력 금지'와 무관하다(개인 사연이 아니라 물건의 정체).
- 지금의 국면이 물건을 정한다(핵심): 약력은 이 인물이 본디 누구인가의 바탕일 뿐, '지금 무엇을 지니고 있는가'는 최근 맥락의 처지·국면·사건이 정한다. 국면이 바뀌면 지니는 것도 바뀐다 — 임무 장부가 국면 따라 달라지듯, 소지품도 그렇다. 그래야 탐색을 거듭하며 모으는 일에 의미가 생긴다. 처지를 무시하고 늘 같은 갈래(제 무구·직업 도구 일습)만 내놓으면 실패다.
- 처지가 자유롭지 못하면(갇힘·빼앗김·쫓김·중상 등) 지닌 것이 거의 없다 — 압수되고 남은 것, 몸에 숨긴 것, 하찮아 지나쳐진 것뿐이다.
- 그 국면 속의 삶이 자연스레 드러나는 물건을 우선하되, 그 삶을 설명문으로 풀어 쓰지 않는다. 시시한 일상 소지품도 섞어 사람 냄새를 남긴다.
