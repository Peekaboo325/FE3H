...
import { callGeminiStream } from './gemini.mjs'; // 교정(2차) 패스 — Flash SSE 스트리밍
...
//  polish=false(앱 '후보정' 끔) → DeepSeek도 교정 패스 없이 1차를 바로 스트리밍한다(빠르되 보정 안 됨).
//  ⚠️ 게이트(머리글 보강)는 호출부가 만들어 넘기고, 닫기·값()도 호출부 몫(여기선 .먹임만 한다).
//    교정(2차)은 Flash SSE 스트리밍 → 델타가 게이트로 실시간 흘러든다.
export async function 본문생성({ client, model, effort, system, messages, 게이트, tag = 'story', 화수 = 0, polish = true }) {
  if (딥시크인가(model)) {
    // 후보정 끔 — 1차(생성)를 바로 스트리밍하고 끝낸다(2차 교정 없음).
    if (!polish) {
    ...
    await callGeminiStream(
      교정프롬프트,
      { maxOutputTokens: 8192, temperature: 0.2, thinkingBudget: 0, tag: `${tag}-교정` },
      (delta) => 게이트.먹임(delta),
    );
    console.log(`[비용] ${tag} 화${화수} 교정(Flash) — Gemini 별도 과금`);
    return;
  }
