...
//    → 1차에서 본문을 만들고(보정 제외), 그 완성본을 입력으로 2차에서 보정지침만 먹여 '교정'시킨다.
//  ⚠️ 게이트(머리글 보강)는 호출부가 만들어 넘기고, 닫기·값()도 호출부 몫(여기선 .먹임만 한다).
//    스트리밍은 마지막 패스만 — 2패스의 1차(초안)는 화면에 안 보내고 버퍼에 담는다(그만큼 첫 글자까지 대기↑).
export async function 본문생성({ client, model, effort, system, messages, 게이트, tag = 'story', 화수 = 0 }) {
  if (딥시크인가(model)) {
    ...
    // 2차 — 교정(보정지침만, 세계관 등 재주입 안 함 → 재작성 priming 차단). 초안을 입력으로 스트리밍 → 게이트.
