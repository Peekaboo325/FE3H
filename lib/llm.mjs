...
export const 교정역할 = {
  ...
  text:
    ...
    '(3) 남은 문장을 \'있는 재료로만\' 더 벼린다(빼기만이 아니라, 있는 것을 정밀하게 다시 렌더한다). ' +
    '약한·막연한 동사·명사를 장면이 이미 보여준 범위 안에서 더 정확한 말로 바꾸고, 같은 길이 문장이 이어지면 끊거나 붙여 리듬을 다듬으며, 그 문단에서 가장 무거운 말이 문장 끝(강세 자리)에 오게 한다. ' +
    '상투어·관용구는 더 구체적인 말로 바꾸되 새 비유는 만들지 않는다. 정점은 여전히 짧고 건조하게 둔다. ' +
    '이 \'벼림\'은 말맛·정밀도·리듬만 올리는 것이지 내용을 보태는 게 아니다. ' +
    '[울타리 — 반드시, (2)·(3)에 모두 적용] 장면에 이미 함축된 것만 쓴다. 초안에 없던 사실·사건·인물·대사·설정·비유·묘사를 단 하나도 지어내거나 더하지 않는다. ' +
