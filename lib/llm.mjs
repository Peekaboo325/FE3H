...
// 교정(윤문) 모델 허용목록 — 앱 설정에서 받은 값 검증(임의 값 차단). 기본 DeepSeek(싸고 절제-안전).
//  Sonnet·Opus로 올리면 사실·사건은 그대로 두고 문장·리듬·정밀도를 더 끌어올린다(문학 천장↑, 비용↑).
//  ⚠️ Flash는 제외 — 교정 voice가 어긋난다(빌더 진단, 과거 Flash 교정 철거).
const 교정모델허용 = new Set(['deepseek-v4-pro', 'claude-sonnet-4-6', 'claude-opus-4-8']);
export const 교정모델 = (m) => (교정모델허용.has(m) ? m : 'deepseek-v4-pro');
...
export const 교정역할 = {
  ...
  text:
    ...
    '아래 [초안]을 위 지침으로 손본다. 손대기 전에 먼저 (속으로) 문제를 짚고 — 정합·인물 심리·시대어·구조 반복·설명 과잉 — 짚인 자리만 고친다. 문장을 더 화려하게 만드는 게 목적이 아니다. 세 가지를 한다. ' +
    ...
    '(3) 남은 문장의 낱말을 그 장면에 더 맞게 고른다(빼기만이 아니라, 낱말을 장면의 결·인과에 맞게 교체까지). ' +
    '약하거나 막연한 동사·명사는 더 정확한 낱말로, 반응·동작이 그 장면의 감정 인과와 어긋나면 그 인과가 부르는 반응의 낱말로, 사물·인물·인상을 가리키는 낱말이 신분·외양·분위기를 덜 담으면 더 담는 낱말로 바꾼다. ' +
    '리듬을 다듬어 가장 무거운 낱말이 문장 끝(강세 자리)에 오게 한다. 정점은 여전히 짧고 건조하게 둔다. ' +
    '여기까지가 낱말 교체다 — 세계에 없는 사실을 주장하는 낱말로는 바꾸지 않고(인상·비유로 읽히는 범위 안), 그 장면에 이미 있는 재료로 성립하는 낱말만 쓴다. 한 문단에 갈아끼우는 낱말은 손에 꼽을 만큼, 멀쩡한 낱말은 둔다. ' +
    '[울타리 — 반드시, (2)·(3)에 모두 적용] 낱말을 더 맞게 교체하는 것까지다. 누가·무엇을·어떤 순서로 했는지(사실·사건·대사의 내용)는 불변, 새 동작·사건·인물·대사·설정·정보·비유·묘사를 단 하나도 지어내거나 더하지 않는다(분량도 안 늘림). ' +
    '분량을 크게 늘리지 않는다. 지침에도 안 걸리고 더 벼릴 것도 없는 멀쩡한 문장은 글자 하나도 건드리지 않고 그대로 둔다. 의심스러우면 두는 쪽을 택한다. ' +
...
export async function 본문교정({ client, model, draft, write, effort = 'medium' }) {
  const stream = client.messages.stream({
    model,
    max_tokens: 8000,
    ...사고옵션(model, effort), // DeepSeek은 effort 무시(고정 사고), Claude(Sonnet/Opus)는 이 값으로 사고 깊이
