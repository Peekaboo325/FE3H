...
import { DEEPSEEK_TUNING } from './worldview.mjs'; // worldview/딥시크_보정지침.md → bake로 구워짐
...
// ── DeepSeek 한정 '본문 절제 보정' ───────────────────────────────────────────
//  지침 텍스트는 worldview/딥시크_보정지침.md(빌더가 직접 고침) → bake로 lib/worldview.mjs(DEEPSEEK_TUNING)에 구워짐.
//  SYSTEM(박제 4종 = 모든 모델 공통)과 별개로, DeepSeek일 때만 시스템 끝에 덧댄다(Opus 프롬프트 불변).
export function 모델별지침(model) {
  return 딥시크인가(model) && DEEPSEEK_TUNING ? [{ type: 'text', text: DEEPSEEK_TUNING }] : [];
}
