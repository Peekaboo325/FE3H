...
export type DailyTrait = {
  name: string; // 사전(TRAITS) 분류 키 — 기계적 정체(흡인·무골 등). 이 값으로 효과를 건다.
  ability?: AbilityKey; // 어느 능력에 거는 특성인가(사전에서 파생)
  label?: string; // 그 인물용 표시 이름(흡인→자색/인망…). 없으면 name 그대로. 연유층(서사 색).
};
...
export type DailyState = {
  ...
  talents?: AbilityKey[]; // 재능(빌더가 ≤3 선택) — 고른 능력은 C, 나머지는 E에서 시작
  start_grades?: Partial<Record<AbilityKey, Grade>>; // 시작 등급(talents에서 파생: 재능=C·나머지=E)
